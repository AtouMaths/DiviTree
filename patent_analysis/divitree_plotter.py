...
class DiviTreePlotter:
    ...
    def build_tooltip(self, row, df=None): # def build_tooltip(self, row, interleaving_type, df=None):
        ...
        if interleaving_type == "Concepts":
            tooltip = f"<b>{prefix}</b><br>"          # prefix = "Concept: optical P disk"
        elif interleaving_type == "Claims":
            tooltip = f"<b>{reference_mode}: {prefix}</b><br>"  # prefix = pub number
        else:
            tooltip = f"<b>{reference_mode}: {prefix}</b><br>"
        ...
        if interleaving_type == "Images":
            ...
            if non_ascii > len(claim_text) * 0.3:
                ...
                # ── Claim text (word-wrapped at 80 chars) ────────────────────
                words = claim_text.split()
                lines = []
                current = []
                current_len = 0
                for word in words:
                    if current_len + len(word) + (1 if current else 0) > 80:
                ...
                # ── XFR-style concept extraction ─────────────────────────────
                try:
                    from patent_analysis.claim_concepts import extract_concepts
                    concepts = extract_concepts(claim_text, language='EN', max_concepts=8)
                    if concepts:
                        tooltip += "<br><br><b>Concepts:</b><br>"
                        for c in concepts:
                            tooltip += f"• {c}<br>"
                except Exception:
                    pass  # concept extraction is best-effort; never abort tooltip
            else:
    ...
    def _prepare_sunburst_df(self, df, interleaving_type, reference_mode):
        ...
        app_types = ("Application", "Applications", "Publications", "Parents", "Priorities")
        for _, row in df.iterrows():
        ...
        if interleaving_type == "Images":
            def _image_tooltip(row):
                ...
                if '|' in pub:
                    pub = pub.split('|', 1)[1]
                if _id.lower().startswith('<img '):
                    # Inject data-pub into existing tag
                    return _id.replace('<img ', f'<img data-pub="{pub}" ', 1)
                if _id.lower().startswith(('http://', 'https://', 'data:image')):
                    return f"<img data-pub=\"{pub}\" src='{_id}' width='320'>"
                return ''
    ...
    def append_image_hover_script(self, html_path):
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function positionBox(clientX, clientY) {
                ...
                // Keep popup inside the viewport with a 10px margin
                var left = Math.min(clientX + 16, vw - boxW - 10);
                var top  = Math.min(clientY + 16, vh - boxH - 10);
