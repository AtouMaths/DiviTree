...
class PatentProcessor:
    ...
    _PROGRESS_HTML = (
        ...
        "}}"
        "</style>"
        "<div style='margin:8px 0 6px 0;font-family:sans-serif'>"
        "<div style='font-size:13px;margin-bottom:4px'>{msg}</div>"
        "<div style='width:480px;height:18px;background:#e0e0e0;border-radius:4px;overflow:hidden'>"
    ...
    # ✅ Step 4: Setup dropdowns and buttons
    def setup_dropdowns_and_buttons(self):
        ...
        action_labels = [
            ...
            "Show Classifications", "Show Parties", "Show Images", "Show Claims", "Show Concepts"
        ]        
        action_flags = [
            'Show_priorities', 'Show_applications', 'Show_parents', 'Show_publications', 'Show_citations',
            'Show_classifications', 'Show_parties', 'Show_images', 'Show_claims', 'Show_concepts'
        ]
        ...
        with output_widget:
            ...
            try:
                ...
                for candidate in [
                    ...
                    os.path.join(os.getcwd(), 'filtered_output'),
                    self.output_dir,
                ...
                if not best:
                    print(f"ℹ️ No {label} DiviTree history found "
                          "(no matching sunburst_plot_*.html in ./output/ or ./filtered_output/).")
                    return
    ...
    def __init__(self, OPSClient, models, exceptions):
        ...
        self.output_dir = os.path.join(os.getcwd(), 'filtered_output')
    ...
    def update_doc_number(self, change):
        ...
        if change['new'] == 'application':
            ...
            self.doc_number_widget.value = '2101496' # 2101496 B1 , 4477148 A2, 3751504 B1, 3242420 A1
            self.country_widget.value = 'EP'
            self.kind_widget.value = 'B1' # 'B1', 'A2', A1
            ...
            with self.text_output:
                ...
                if os.path.exists(tree_path):
                    with open(tree_path, 'r') as f:
                        content = f.read()
                    print(f"Raw content of {self.selected_tree_type.upper()} tree file:\n{content}")
                    print(f"{self.selected_tree_type.upper()} tree contains {len(content.splitlines())} lines.\n")
                    # ── Auto-copy tree text to clipboard (text embedded directly in JS)
    ...
    def run_divitree_chart(self, tree_path=None):
        ...
        try:
            plotter = DiviTreePlotter(workdir="./output")
