...
class FamilyRecord:
    ...
    def _parse_priority_claims(
        ...
        # ========================================================================
        # PHASE 2: DETERMINE ORAP ONCE AFTER ALL PRIORITIES COLLECTED
        # Now we have ALL priorities and can make the right decision
        # ========================================================================
        
        # print("output index + app_number_full + priority_doc_number_full:", index, app_number_full, priority_doc_number_full)
        # print("app_kind_text, app_country_text, priority_country:", app_kind_text, app_country_text, priority_country)        
        if not app_data['orap']:
            if app_data['priority_dates']:
                # Get the priority with the EARLIEST date (oldest priority = ORAP)
                # Use priority number as tiebreaker for deterministic results
                earliest_priority = min(
                    app_data['priority_dates'].items(), 
                    key=lambda x: (x[1], x[0])  # Sort by (date, priority_number)
                )[0]
                
                # ALL applications (including WO/PCT) point to their earliest priority
                # WO apps should NOT point to themselves (causes tree duplication)
                app_data['orap'] = [earliest_priority]
                
                # # Debug: Show WO ORAP assignments to verify consistent JP roots
                # if app_kind_text == 'W' or app_country_text == 'WO':
                #     print(f"  WO {app_number_full} → ORAP: {earliest_priority} (date: {app_data['priority_dates'][earliest_priority]})")

            else:
                # Fallback: if no priorities found, use current app as root
    ...
    def _parse_xml_to_dataframe(
        ...
        # Step 1: process bibliographic + legal event info
        ...
        #         print(f"    priority_numbers: {app_data.get('priority_numbers', [])[:3]}...")  # First 3

        # # Step 2: SUBSET-BASED ORAP ASSIGNMENT
        # print("\n🔍 DEBUG: About to call _assign_orap_by_subset")
        # self._assign_orap_by_subset(data)
        
        # Step 2b: US/CN/JP/KR divisional hierarchy (filing-date-based)
    ...
    def get_filtered_application_numbers(self, additional_countries: Optional[str] = None) -> Optional[pd.DataFrame]:
        ...
        if not filtered_df.empty:
            # CRITICAL: Preserve ORAP values that were set by specialized logic:
            # - Divisionals: ORAP set by _assign_divisional_hierarchy (subset logic) 
            # - WO/PCT: ORAP set to earliest priority (prevents self-loops)
            # Only call sort_orap for apps that haven't had their ORAP specially set
            
            # Get the set of apps that had subset-based ORAP assignment
            subset_apps = getattr(self, 'subset_assigned_apps', set())
            if subset_apps:
                # Split: apply sort_orap only to rows NOT already assigned by divisional logic
                ...
                # Recombine preserving original row order
                filtered_df = pd.concat(
                    [non_subset, filtered_df[subset_mask]], ignore_index=False
                ).sort_index()
            else:
                # No subset-assigned apps at all — apply sort_orap to everything
