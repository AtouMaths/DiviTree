...
class TreeProcessor:
    ...
    def __init__(
        ...
        # self.workdir = "/home/jovyan/output" # The directory where files will be stored.
        self.workdir = "./output"
    ...
    def process_concepts(self, current_root, level, tree_file):
        ...
        for concept in concepts:
            tree_file.write(f"{indent}[Concept] {concept}\n")
    ...
    def _fetch_register_data(self, pub_number: str,
        ...
        try:
            ...
            if not flattened_rows:
                print(f"No reg:register-document entries found for {pub_number}")
                return None
    ...
    def format_citations(self, citations_list):
        ...
        return str(citations_df['formatted_citation'].dropna().tolist()) # return ', '.join(citation_list) 
    ...
    def format_classifications(self, classification_data):
        ...
        return str(classification_data)  # return ', '.join(classification_data) 
    ...
    def extract_party_names(self, biblio_df):
        ...
        def is_valid(name):
            # Keep names that either include [XX] (country code) or have at least two words and no lone comma
            return (
                '[' in name or
                (',' not in name and len(name.split()) >= 2)
            )
