...
UNDERSTOOD UNLESS UNTIL UP UPBEAT UPON UPRIGHT UPSIDE-DOWN UPWARD USE USED
...
_REF_PATTERNS = [
    re.compile(r'\(\d+\)'),             # (766)
    re.compile(r'\([A-Za-z0-9]+\)'),    # (VOB), (STCc)
...
def concepts_strategy(chunked_text: str, max_words_per_concept: int = 6) -> List[str]:
    ...
    for chunk in raw_chunks:
        ...
        for raw_word in words:
            # Clean hyphens used as compound joiners
            ...
            upper = word.upper()
            if _is_full_stopword(upper):
                continue
            if _is_ti_stopword(upper):
                continue
            if len(word) <= 2:
                continue
